"""Client-side player model for the package runtime."""

import pygame

from better_together_shared.asset_catalog import CANNONBALL_ASSET_ID, OBSTACLE_ASSET_ID
...
class Player:
    def __init__(self, x, y, width, height, char):
        self.player = (self.x, self.y, self.width, self.height, self.animation) = (
            ...
            width,
            height,
            list([(width * frame, 2 * height, width, height) for frame in range(3)])[1],
        )
        self.char = char
        self.frame = 0
        self.velocity = 2
        self.increment = 0
        (self.maxHeight, self.maxWidth) = (WINDOW_HEIGHT, WINDOW_WIDTH)
    ...
    def collision(self, playerImg):
        ...
        cropped.blit(
            ...
            list(
                [
                    (self.width * frame, 2 * self.height + 3 * self.height // 4, self.width, self.height // 4)
                    for frame in range(3)
                ]
            )[1],
        )

        obstacle = load_scaled_image(OBSTACLE_ASSET_ID, (self.maxWidth // 2, self.maxHeight))
